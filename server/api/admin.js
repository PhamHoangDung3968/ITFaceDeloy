...
router.get('/export-tk-teacher/:teacherID/:termID/:classCode/:totalNullTimes', async (req, res) => {
  try {
      const { teacherID, termID, classCode, totalNullTimes } = req.params;

      // Export the class sections to an Excel buffer
      const buffer = await StatisticDAO.exportTKClassSectionsToExcel(teacherID, termID, parseInt(totalNullTimes), classCode);
...
router.get('/export-tk-bcnk/:termID/:classCode/:totalNullTimes', async (req, res) => {
  try {
      const { termID, classCode, totalNullTimes } = req.params;

      // Export the class sections to an Excel buffer
      const buffer = await StatisticDAO.exportTKClassSectionsToExcelWithoutTeacherID(termID, parseInt(totalNullTimes), classCode);
      ...
      // Set headers and send the file
      res.setHeader('Content-Disposition', `attachment; filename=statistic_attendance_${classCode}_buoi${totalNullTimes}.xlsx`);
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      stream.pipe(res);
  } catch (error) {
      console.error('Error exporting attendance:', error);
...
router.post('/register_user', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://ce59-2001-ee0-4f80-9720-c97d-615c-39dc-ae46.ngrok-free.app/api/register', { name, image });
...
router.post('/re_register_user', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://ce59-2001-ee0-4f80-9720-c97d-615c-39dc-ae46.ngrok-free.app/api/re_register', { name, image });
...
router.post('/login_user', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://ce59-2001-ee0-4f80-9720-c97d-615c-39dc-ae46.ngrok-free.app/api/login', { name, image });
...
router.post('/check_user', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://ce59-2001-ee0-4f80-9720-c97d-615c-39dc-ae46.ngrok-free.app/api/check_user', { name });
...
router.get('/count_users_regist', async (req, res) => {
  try {
    ...
    const response = await axios.get('https://ce59-2001-ee0-4f80-9720-c97d-615c-39dc-ae46.ngrok-free.app/api/count-pickle-files');
